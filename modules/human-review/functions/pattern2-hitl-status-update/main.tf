...
locals {
  ...
  module_instance_id = substr(md5("${path.module}-pattern2-hitl-status-update"), 0, 8)
}
...
resource "random_id" "build_id" {
  ...
  keepers = {
    ...
    content_hash       = md5("pattern2-hitl-status-update-function")
  }
}
...
# IAM role
resource "aws_iam_role" "pattern2_hitl_status_update_role" {
...
# IAM policy
resource "aws_iam_policy" "pattern2_hitl_status_update_policy" {
...
resource "aws_iam_role_policy_attachment" "pattern2_hitl_status_update_policy_attachment" {
  role       = aws_iam_role.pattern2_hitl_status_update_role.name
  policy_arn = aws_iam_policy.pattern2_hitl_status_update_policy.arn
}
...
resource "aws_iam_policy" "kms_policy" {
  ...
  description = "KMS policy for Pattern-2 HITL status update function"
...
resource "aws_iam_role_policy_attachment" "pattern2_hitl_status_update_kms_attachment" {
  count      = var.encryption_key_arn != null ? 1 : 0
  role       = aws_iam_role.pattern2_hitl_status_update_role.name
...
resource "aws_iam_policy" "vpc_policy" {
  ...
  description = "VPC policy for Pattern-2 HITL status update function"
...
resource "aws_iam_role_policy_attachment" "pattern2_hitl_status_update_vpc_attachment" {
  count      = length(var.vpc_subnet_ids) > 0 ? 1 : 0
  role       = aws_iam_role.pattern2_hitl_status_update_role.name
...
resource "aws_cloudwatch_log_group" "pattern2_hitl_status_update_logs" {
  name              = "/aws/lambda/${aws_lambda_function.pattern2_hitl_status_update.function_name}"
