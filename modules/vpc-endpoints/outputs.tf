...
output "appsync_api_endpoint_id" {
  description = "ID of the appsync-api interface endpoint, or null when not enabled. Required when AppSync visibility is PRIVATE."
  value       = try(aws_vpc_endpoint.interface["appsync-api"].id, null)
}
