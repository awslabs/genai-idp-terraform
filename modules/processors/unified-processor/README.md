...
| <a name="input_use_bda"></a> [use\_bda](#input\_use\_bda) | Runtime processing-path selector set by the instantiating façade: true routes documents through the Bedrock Data Automation (BDA) branch; false routes through the Bedrock-LLM/SageMaker pipeline branch. Required — the engine is never instantiated without an explicit value. (The state-machine routing that consumes this flag is wired in a follow-up task.) | `bool` | n/a | yes |
