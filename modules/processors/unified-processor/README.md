...
| <a name="input_bda_project_arn"></a> [bda\_project\_arn](#input\_bda\_project\_arn) | Optional Bedrock Data Automation project ARN used ONLY as the fallback<br>`BdaProjectArn` for `additional_configurations` versions that set<br>`use_bda: true` without their own per-version `bda_project_arn` (threaded to<br>`processor-configuration` as `fallback_bda_project_arn`). It does NOT relink<br>the `default` configuration version, so pipeline façades can link extra<br>BDA versions while keeping their default pipeline (R5.6). This is a seeding<br>fallback, distinct from the removed deploy-time branch gate — both branches<br>are always deployed regardless of this value. | `string` | `null` | no |
...
| <a name="input_default_bda_project_arn"></a> [default\_bda\_project\_arn](#input\_default\_bda\_project\_arn) | Optional Bedrock Data Automation project ARN used to LINK the `default`<br>configuration version to a BDA project during seeding (threaded to<br>`processor-configuration`). Set by the `bda-processor` façade so BDA<br>routing works out of the box; it also serves as the last-resort fallback<br>for that façade's own `use_bda: true` additional versions. Pipeline façades<br>leave this null so their default stays pipeline (R5.6). This is a seeding<br>input only — it does NOT gate whether the BDA branch is deployed (both<br>branches are always deployed). | `string` | `null` | no |
...
| <a name="output_state_machine_start_at"></a> [state\_machine\_start\_at](#output\_state\_machine\_start\_at) | The StartAt state of the document-processing state machine. Always 'RouteByProcessingMode': both branches are always deployed and each document is routed at runtime by its config version's use\_bda flag. |
| <a name="output_state_machine_state_names"></a> [state\_machine\_state\_names](#output\_state\_machine\_state\_names) | The set of state names in the document-processing state machine definition. Includes both the BDA-branch and pipeline-branch states (both branches are always deployed). |
...
| <a name="output_state_machine_arn"></a> [state\_machine\_arn](#output\_state\_machine\_arn) | ARN of the Step Functions state machine for document processing |
| <a name="output_state_machine_name"></a> [state\_machine\_name](#output\_state\_machine\_name) | Name of the Step Functions state machine for document processing |
| <a name="output_state_machine_start_at"></a> [state\_machine\_start\_at](#output\_state\_machine\_start\_at) | The StartAt state of the document-processing state machine. Always 'RouteByProcessingMode'; documents route at runtime by their config version's use\_bda flag. |
