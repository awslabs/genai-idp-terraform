...
| <a name="input_rule_validation_memory_size"></a> [rule\_validation\_memory\_size](#input\_rule\_validation\_memory\_size) | Memory (MB) for the three rule-validation Lambdas — rule-validation,<br>rule-validation-orchestration, and rule-validation-policy-class (created only<br>when enable\_rule\_validation = true). Defaults to 4096 (upstream). Lower it<br>for accounts whose Lambda per-function memory service quota is below 4096 MB<br>(some sandbox accounts cap at 3008 MB). | `number` | `4096` | no |
