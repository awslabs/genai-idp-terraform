...
variable "rule_validation_memory_size" {
  description = <<-EOT
    Memory (MB) for the three rule-validation Lambdas — rule-validation,
    rule-validation-orchestration, and rule-validation-policy-class (created only
    when enable_rule_validation = true). Defaults to 4096 (upstream). Lower it
    for accounts whose Lambda per-function memory service quota is below 4096 MB
    (some sandbox accounts cap at 3008 MB).
  EOT
  type        = number
  default     = 4096
