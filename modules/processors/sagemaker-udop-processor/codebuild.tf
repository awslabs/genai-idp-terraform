  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [
          "arn:${data.aws_partition.current.partition}:logs:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:log-group:/aws/codebuild/${var.name}-udop-processor-build-${random_string.suffix.result}",
          "arn:${data.aws_partition.current.partition}:logs:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:log-group:/aws/codebuild/${var.name}-udop-processor-build-${random_string.suffix.result}:*"
        ]
      },
      {
...
resource "aws_codebuild_project" "udop_processor_build" {
  ...
  environment {
    ...
    environment_variable {
      name  = "ECR_URI"
      value = aws_ecr_repository.udop_processor.repository_url
    }
  ...
  logs_config {
    cloudwatch_logs {
      group_name = "/aws/codebuild/${var.name}-udop-processor-build-${random_string.suffix.result}"
      status     = "ENABLED"
    }
  }
