...
| <a name="input_rule_validation_memory_size"></a> [rule\_validation\_memory\_size](#input\_rule\_validation\_memory\_size) | Memory (MB) for the rule-validation Lambdas (only created when enable\_rule\_validation = true). Defaults to 4096; lower for accounts whose Lambda memory quota caps below 4096 MB. | `number` | `4096` | no |
